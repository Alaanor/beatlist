...
import { PlaylistBase, PlaylistLocalMap, PlaylistMapImportError } from '@/libraries/playlist/PlaylistLocal';
import Base64SrcLoader from '@/libraries/os/utils/Base64SrcLoader';
import Progress from '@/libraries/common/Progress';
import PlaylistDeserializeBeatsaverBeatmap
  from '@/libraries/playlist/loader/deserializer/helper/PlaylistDeserializeBeatsaverBeatmap';
  ...
  public async deserialize(progress?: Progress): Promise<PlaylistBase> {
    ...
    try {
      ...
      return {
        ...
        maps: await JsonDeserializer.convertToHash(json.songs, progress ?? new Progress()),
      } as PlaylistBase;
    } catch (e) {
      throw INVALID_JSON;
    }
  ...
  private static getErrorFor(item: BeatsaverItem):
    ...
    switch (item.loadState.errorType) {
      case BeatsaverItemLoadError.BeatmapNotOnBeatsaver:
        return { error: PlaylistMapImportError.BeatsaverInexistent, errorInfo: 'Beatmap not found on beatsaber' };
      case BeatsaverItemLoadError.BeatsaverServerNotAvailable:
        return { error: PlaylistMapImportError.BeatsaverRequestError, errorInfo: 'Server not available' };
      case BeatsaverItemLoadError.InvalidDataReceivedFromBeatsaver:
        return { error: PlaylistMapImportError.BeatsaverRequestError, errorInfo: 'Unexpected data received from beatsaber' };
      case BeatsaverItemLoadError.BeatsaverRateLimited:
        return { error: PlaylistMapImportError.BeatsaverRequestError, errorInfo: 'Reached the rate limit' };
