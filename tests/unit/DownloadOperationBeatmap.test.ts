...
jest.mock('@/libraries/net/downloader/DownloadUnit', () => jest.fn()
...
describe('download operation beatmap', () => {
  ...
  it('should download the beatmap', async () => {
    ...
    jest.spyOn(BeatSaber, 'GetFolderPathFor').mockImplementation().mockReturnValue(fakeInstallationLevelsPath);

    await new Promise((done) => {
      const operationBeatmap = new DownloadOperationBeatmap(beatmap);

      operationBeatmap.OnCompleted(async (result: DownloadOperationBeatmapResult) => {
        const fooFile = (await fs.readFile(destinationFooTxtPath)).toString();

        expect(result.status).toBe(DownloadOperationBeatmapResultStatus.Success);
