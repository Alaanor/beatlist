...
describe("beatsaver api", () => {
  it("should find the correct map by hash", async () => {
    ...
    const response = (await BeatsaverAPI.Singleton.getBeatmapByHash(
      "152da020c774105db2d70fcbfd4991ef2878384b"
    )) as BeatSaverAPIResponseDataFound<BeatsaverBeatmap>;
  ...
  it("should search at beatsaverPage 2 for the given search value", async () => {
    ...
    const searchResult = (await BeatsaverAPI.Singleton.searchBeatmaps(
      "speed",
      2
    )) as BeatSaverAPIResponseDataFound<BeatsaverPage>;
